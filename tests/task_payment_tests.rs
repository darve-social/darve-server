...
test_with_server!(
    ...
    |server, state, config| {
        ...
        let response = server
            ...
            .add_header("Authorization", format!("Bearer {}", p1_token))
            .add_header("Accept", "application/json")
            .await;
        response.assert_status_success();

        let _ = task_helpers::success_deliver_task(&server, &task_id, &p1_token)
            .await
            .unwrap();

        let response = server
            .post(&format!("/api/tasks/{}/accept", task_id))
            .add_header("Authorization", format!("Bearer {}", p2_token))
            .add_header("Accept", "application/json")
            .await;
        response.assert_status_success();

        let _ = task_helpers::success_deliver_task(&server, &task_id, &p2_token)
            .await
            .unwrap();

        let response = server
            .post(&format!("/api/tasks/{}/accept", task_id))
            .add_header("Authorization", format!("Bearer {}", p3_token))
            .add_header("Accept", "application/json")
            .await;
        response.assert_status_success();

        let _ = task_helpers::success_deliver_task(&server, &task_id, &p3_token)
            .await
            .unwrap();
...
test_with_server!(one_donor_and_has_not_delivered, |server, state, config| {
    ...
    let response = server
        .post(&format!("/api/tasks/{}/accept", task_id))
        .add_header("Authorization", format!("Bearer {}", ptoken))
        .add_header("Accept", "application/json")
        .await;
    response.assert_status_success();
    let task_thing = Thing::try_from(task_id.as_str()).unwrap();
...
test_with_server!(two_donor_and_has_not_delivered, |server, state, config| {
    ...
    let endow_user_response = server
        ...
        .add_header("Authorization", format!("Bearer {}", donor1_token))
        .add_header("Accept", "application/json")
        .await;
    endow_user_response.assert_status_success();
...
test_with_server!(five_donor_and_has_not_delivered, |server, state, config| {
    ...
    let endow_user_response = server
        ...
        .add_header("Authorization", format!("Bearer {}", donor2_token))
        .add_header("Accept", "application/json")
        .await;
    endow_user_response.assert_status_success();
    let participate_response = server
    ...
    let endow_user_response = server
        ...
        .add_header("Authorization", format!("Bearer {}", donor3_token))
        .add_header("Accept", "application/json")
        .await;
    endow_user_response.assert_status_success();
    let participate_response = server
    ...
    let endow_user_response = server
        ...
        .add_header("Authorization", format!("Bearer {}", donor4_token))
        .add_header("Accept", "application/json")
        .await;
    endow_user_response.assert_status_success();
    let participate_response = server
