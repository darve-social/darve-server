...
test_with_server!(
    ...
    |server, ctx_state, config| {
        ...
        let post = create_post(server, &default_discussion, data, &user2_token).await;
...
test_with_server!(create_post_with_users_test, |server, ctx_state, config| {
    ...
    let response = create_post(server, &default_discussion, data, &user_token).await;
    response.assert_status_forbidden();
...
test_with_server!(
    ...
    |server, ctx_state, config| {
        ...
        let response = create_post(server, &discussion.id, data, &user1_token).await;
        response.assert_status_forbidden();
...
test_with_server!(
    ...
    |server, ctx_state, config| {
        ...
        let response = create_post(server, &disc_id, data, &user1_token).await;
        response.assert_status_ok();
        let post = response.json::<PostView>();
        assert_eq!(post.r#type, PostType::Idea);
    }
