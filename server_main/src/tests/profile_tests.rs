...
mod tests {
    ...
    async fn get_user_chat() {
        ...
        let discussion_posts = get_discussion_view(&ctx_state._db, &ctx, chat_disc_id.clone(), DiscussionParams {
            topic_id: None,
            start: None,
            count: None,
        }).await.expect("discussion");
        assert_eq!(discussion_posts.posts.len(), 1);
        assert_eq!(discussion_posts.posts.get(0).expect("post").title, post_name);
