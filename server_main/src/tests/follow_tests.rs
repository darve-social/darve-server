...
mod tests {
    ...
    use sb_community::routes::profile_routes::{ProfileChat, ProfileChatList, ProfileDiscussionView, ProfilePage};
    ...
    async fn get_user_followers() {
        ...
        let followers_nr = follow_db_service.user_followers_number(user1_id.clone()).await.expect("user 1 followers nr");
        assert_eq!(0, followers_nr);

        let is_following = follow_db_service.is_following(user1_id.clone(), user2_id.clone()).await.expect("is_following");
        ...
        let is_following = follow_db_service.is_following(user1_id.clone(), user2_id.clone()).await.expect("is_following");
        assert_eq!(is_following, false);
        let is_following = follow_db_service.is_following(user2_id.clone(), user1_id.clone()).await.expect("is_following");
        ...
        let create_post = server.post("/api/user/post").multipart(MultipartForm::new().add_text("title", post_name.clone()).add_text("content", "contentttt").add_text("topic_id", "")).await;
        let created = create_post.json::<CreatedResponse>();
        &create_post.assert_status_success();
        assert_eq!(created.id.len() > 0, true);

        let response = server.get(format!("/api/user/{}/posts", username1).as_str()).await;
        let parsed_response = response.json::<ProfileDiscussionView>();
        &create_post.assert_status_success();
        assert_eq!(parsed_response.posts.len(), 1);
        assert_eq!(parsed_response.posts[0].username.clone().unwrap(), username1);

        // login user3
        server.get("/logout").await;
        let login_response = server.post("/api/login").json(&LoginInput { username: username3.clone(), password: "some3242paSs#$".to_string(), next: None }).await;
        login_response.assert_status_success();
        // user3 get followers stream
        let create_response = server.get("/u/following/posts").await;
        dbg!(&create_response);
