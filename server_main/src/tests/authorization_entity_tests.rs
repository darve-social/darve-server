...
mod tests {
    ...
    async fn authorize_save() {
        ...
        let is_auth = acc_right_service.is_authorized(&Thing::try_from(user_ident.clone()).unwrap(), &authorization_comm_owner).await;
        assert_eq!(is_auth.is_ok(), true);

        let auth = acc_right_service.authorize(Thing::try_from(user_ident.clone()).unwrap(), authorization, None).await;
        assert_eq!(auth.is_ok(), true);
        let authorization2 = Authorization::new(auth_rec2.clone(), AUTH_ACTIVITY_VISITOR.to_string(), 101);
        dbg!(&authorization2);
        let auth = acc_right_service.authorize(Thing::try_from(user_ident.clone()).unwrap(), authorization2.clone().unwrap(), None).await;
        dbg!(&auth);
        assert_eq!(auth.is_ok(), true);

        let mut authorizations = acc_right_service.get_authorizations(&Thing::try_from(user_ident.clone()).unwrap()).await.unwrap();
        let mut auth_iter = authorizations.iter_mut();
        let is_authorized = acc_right_service.is_authorized(&Thing::try_from(user_ident.clone()).unwrap(), &authorization2.clone().unwrap()).await;
