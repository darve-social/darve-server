...
use sb_middleware::utils::db_utils::{exists_entity, get_entity, get_entity_view, with_not_found_err, IdentIdName, ViewFieldSelector};
...
pub struct Community {
    ...
    pub title: String,
    pub name_uri: String,
    pub main_discussion: Option<Thing>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl<'a> CommunityDbService<'a> {
    ...
    DEFINE FIELD title ON TABLE {TABLE_NAME} TYPE string;
    ...
    DEFINE FIELD main_discussion ON TABLE {TABLE_NAME} TYPE option<record<{DISCUSSION_TABLE_NAME}>>;
...
");
    ...
    pub async fn create_update(&self, mut record: Community) -> CtxResult<Community> {
        ...
        let comm: Option<Community> = self.db
        ...
        if comm.main_discussion.is_none() {
            let disc = self.db
            ...
            let comm_upd: CtxResult<Option<Community>> = self.db
        ...
        aright_db.authorize(comm.created_by.clone(), auth1, None).await?;
...
// or like discussion_entity.add_topic
        ...
        let user_comm = match user_comm {
            ...
            Err(err) => {
                if let AppError::EntityFailIdNotFound { .. } = err.error {
                    return Err(AppError::Generic { description: "User profile needs discussion to add chat".to_string() });
                }
        ...
        if !p_chats.contains(&discussion_id) {
            ...
            let res:Option<Community>=self.db.update((comm_id.tb, comm_id.id.to_string()))
                .patch(PatchOp::add("/profile_chats".to_string().as_str(), [discussion_id]))
                .await?;
        }
        Ok(())
    }
