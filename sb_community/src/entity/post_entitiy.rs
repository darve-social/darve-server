...
use sb_middleware::utils::db_utils::{exists_entity, get_entity, get_entity_list_view, get_entity_view, IdentIdName, Pagination, QryOrder, ViewFieldSelector, with_not_found_err};
...
");/*
    ...
    pub async fn get_by_discussion_desc_view<T: for<'b> Deserialize<'b> + ViewFieldSelector>(&self, discussion_id: Thing, params: DiscussionParams) -> CtxResult<Vec<T>> {
        let filter_by = Self::create_filter(discussion_id, params.topic_id);
        let pagination = Some(Pagination { order_by: Option::from("r_created".to_string()), order_dir: Some(QryOrder::DESC), count: params.count.unwrap_or(20), start: params.start.unwrap_or(0) });
    ...
    fn create_filter(discussion_id: Thing, topic: Option<Thing>) -> IdentIdName {
        let filter_discussion = IdentIdName::ColumnIdent { column: "belongs_to".to_string(), val: discussion_id.to_raw(), rec: true};
        let filter_by = match topic {
            None => filter_discussion,
            Some(topic_val) => IdentIdName::ColumnIdentAnd(vec![filter_discussion, IdentIdName::ColumnIdent { column: TABLE_COL_TOPIC.to_string(), val: topic_val.to_raw(), rec: true }])
        };
        filter_by
    }
    ...
    pub async fn set_media_url(&self, post_id: Thing, url: &str) -> CtxResult<Post> {
        ...
        let res: Option<Post>  = self.db.update((post_id.tb.clone(), post_id.id.clone().to_string()))
