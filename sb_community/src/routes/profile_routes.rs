...
use sb_middleware::utils::db_utils::{
    get_entities_by_id, record_exists, IdentIdName, ViewFieldSelector,
};
...
async fn display_profile(
    ...
    let mut dis_view = DiscussionDbService {
        db: &ctx_state._db,
        ctx: &ctx,
    }
    .get_view::<ProfileDiscussionView>(IdentIdName::Id(disc_id.clone()))
    .await?;

    let discussion_posts = PostDbService {
        db: &ctx_state._db,
        ctx: &ctx,
    }
...
async fn get_chats(
    ...
    ctx.to_htmx_or_json_res(ProfileChatList {
        user_id,
        discussions,
    })
}
...
async fn get_create_chat_discussion(
    ...
    ctx.to_htmx_or_json_res(ProfileChat {
        discussion,
        user_id,
    })
}
