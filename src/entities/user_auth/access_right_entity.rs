...
pub struct AccessRight {
    ...
    #[serde(
        ...
        serialize_with = "serialize_chrono_as_sql_datetime"
    )]
...
fn serialize_chrono_as_sql_datetime<S>(
...
const TABLE_NAME_ACCESS_RULE: &str = access_rule_entity::TABLE_NAME;
const TABLE_NAME_LOCAL_USER: &str = local_user_entity::TABLE_NAME;
const TABLE_NAME_PAYMENT_ACTION: &str = access_gain_action_entity::TABLE_NAME;
...
");
    ...
    pub async fn list_by_user(&self, user_id: &Thing) -> CtxResult<Vec<AccessRight>> {
        get_entity_list::<AccessRight>(
            self.db,
            TABLE_NAME.to_string(),
            &IdentIdName::ColumnIdent {
                column: TABLE_NAME_LOCAL_USER.to_string(),
                val: user_id.to_raw(),
                rec: true,
            },
            None,
    ...
    pub async fn create_update(&self, mut record: AccessRight) -> CtxResult<AccessRight> {
        ...
        let acc_right: Option<AccessRight> = self
            .db
            ...
            if let Some(_user_auth_ge) = auth_list.iter().find(|a| {
                ...
                a.authorize_record_id.eq(&compare_parent_id)
                    && a.ge_equal_ident(&Authorization {
                        ...
                        authorize_height: authorization.authorize_height,
                    })
                    .unwrap_or(false)
            }) {
                return Ok(true);
    ...
    pub async fn get_authorizations(&self, user_id: &Thing) -> CtxResult<Vec<Authorization>> {
        ...
        let authorizations = access_rights
            .iter()
            .map(|a_right| {
                if a_right
                    .expires_at
                    .unwrap_or(now.add(Duration::from_secs(1)))
                    > now
                {
    ...
    pub async fn is_authorized(
        ...
        if !res {
            return Err(self.ctx.to_ctx_error(AppError::AuthorizationFail {
                required: authorization.clone().into(),
            }));
        }
        Ok(())
    }

    pub async fn add_paid_access_right(
        &self,
        local_user_id: Thing,
        access_rule_id: Thing,
        payment_action_id: Thing,
    ) -> CtxResult<AccessRight> {
        ...
        // TODO in transaction
        let paid_access_rule = AccessRuleDbService {
            db: self.db,
            ctx: self.ctx,
        }
        .get(IdentIdName::Id(access_rule_id.clone()))
        .await?;

        let access_rights = self.list_by_user(&local_user_id).await?;
        let existing_access_right = match access_rights.len() > 0 {
            true => access_rights.into_iter().find(|a_right| {
                a_right
                    .authorization
                    .eq(&paid_access_rule.authorization_required)
            }),
            false => None,
        };
        ...
        let save_access_right = match existing_access_right {
            None => {
                let expires_at: Option<DateTime<Utc>> = match paid_access_rule.available_period_days
                {
                    None => None,
                    Some(lasts_days) => {
                        Some(Utc::now().add(Duration::from_secs(lasts_days * 60 * 60 * 24)))
                    }
                };
                AccessRight {
                    id: None,
            ...
            Some(mut a_right) => {
                ...
                let next_expires_date = match paid_access_rule.available_period_days {
                    None => None,
                    Some(extend_for_days) => Option::from(
                        current_extended_to
                            .add(Duration::from_secs(extend_for_days * 60 * 60 * 24)),
                    ),
                };
